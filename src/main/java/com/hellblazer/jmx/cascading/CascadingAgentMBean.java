...
public interface CascadingAgentMBean {
     ...
     * <CODE>CascadingAgent</CODE>. This is the number of source MBeans that
     * have been mounted by this cascading agent in the target MBeanServer.
     * 
     ...
     * cascaded by this <CODE>CascadingAgent</CODE>. The <tt>ObjectInstance</tt>
     * objects returned are to be interpreted in the context of the source
     * MBeanServer: the <tt>ObjectNames</tt> correspond to the
     * <tt>ObjectNames</tt> of the source MBeans in the source MBeanServer.
     * 
     * @return a {@link Set} containing all {@link ObjectInstance
     *         ObjectInstances} representing the cascaded source MBeans.
     */
     ...
     * <p>
     * Whenever possible, this description string should identify the source
     * <tt>MBeanServer</tt> which is cascaded by this <tt>CascadingAgent</tt>,
     * and the semantics of this cascading agent. <br>
     * For instance, if the cascaded agent has a human readable
     * <tt>JMXServiceURL</tt> (i.e. not the form containing the encoded stub),
     * then the source agent could be identified by that URL, and that URL could
     * be used in this description string.<br>
     * Alternatively, if the cascaded agent connector was retrieved from a
     * naming service, then the JMX Agent Name of the cascaded agent could be
     * used to identify it.
     * </p>
     * <p>
     ...
     * MBean names must satisfy in order to be cascaded. This pattern is to be
     * evaluated in the context of the source MBeanServer.
     * 
    ...
    /**
     * Returns the source {@link QueryExp} query filter that the source MBean
     * names must satisfy in order to be cascaded. This query is to be evaluated
     * in the context of the source MBeanServer.
     * 
    ...
    /**
     * Gets the targetPath. This is the <i>domain path</i> under which the
     * source MBeans will be mounted in the target <tt>MBeanServer</tt>. If this
     * attribute is not <tt>null</tt>, all source MBean names will be
     * transformed in the target <tt>MBeanServer</tt> by prefixing their domain
     * name with the string <tt><i>targetPath</i>+"/"</tt>. <br>
     * An MBean whose name is <tt>"D:k1=v1,k2=v2"</tt> will thus be mounted as
     ...
     * it could cause name conflicts in the target <tt>MBeanServer</tt></b>. <br>
     * Similarly, MBeans from different sources should not be mounted under the
     * same <var>targetPath</var>. Moreover, an application should not attempt
     * to mount source MBeans under a <var>targetPath</var> that already contain
     * MBeans in the target <tt>MBeanServer</tt>.
     * 
     ...
     * <p>
     * This is equivalent to calling <tt>start(true)</tt>
     * </p>
     * <p>
     ...
     *                started (for instance, a stop operation is still in
     ...
     *            ignored: MBeans from the source MBeanServer whose name would
     *            cause a conflict in the target MBeanServer are simply not
     *            cascaded.
     * 
     * @exception IOException
     ...
     **/
    public void start(boolean conflictAllowed) throws IOException,
     ...
     * the <tt>CascadingAgent</tt> becomes inactive (see {@link #isActive
     * isActive()}).
     * </p>
     * 
     * @exception IOException
