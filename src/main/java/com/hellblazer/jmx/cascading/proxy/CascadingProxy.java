 ...
 * <tt>MBeanServer</tt>. Connections to that source <tt>MBeanServer</tt> are
 * obtained through an {@link MBeanServerConnectionFactory}. The
 * <tt>CascadingProxy</tt> is a {@link DynamicMBean} that forwards all the calls
 * it receive to a source MBean, residing in a source <tt>MBeanServer</tt>. The
 * <tt>CascadingProxy</tt> also makes it possible for
 * {@link NotificationListener Notification Listeners} to register for
 * notifications emitted by the source MBeans.
 * 
 * @since Java DMK 5.1
 **/
public class CascadingProxy implements DynamicMBean, MBeanRegistration,
        NotificationEmitter {
     ...
     * @param mbscf
     *            An <tt>MBeanServerConnectionFactory</tt> from which
     *            connections with the <tt>MBeanServer</tt> containing the
     *            source MBean can be obtained. The <tt>CascadingProxy</tt> will
     *            call
     *            {@link MBeanServerConnectionFactory#getMBeanServerConnection()
     *            mbscf.getMBeanServerConnection()} every time it needs to
     *            access the subagent.
     **/
    ...
    public Object invoke(String actionName, Object params[], String signature[])
                                                                                ...
                                                                                ReflectionException {
        try {
            return connection().invoke(source, actionName, params, signature);
        } catch (IOException x) {
     ...
     * registered in the MBean server. This default implementation simply checks
     * that the given <var>name</var> is not null, and cache it in the
     * <tt>CascadingProxy</tt>. Once the <tt>CascadingProxy</tt> is registered,
     * its <tt>ObjectName</tt> can be later obtained by calling
     * {@link #getTargetName()}.
     * 
     * @param server
    ...
    /**
     * Removes a listener from the source MBean. If the listener has been
     * registered with different handback objects or notification filters, all
     * entries corresponding to the listener will be removed.
     * 
     * @param listener
    ...
    /**
     * <p>
     * Removes a listener from the source MBean. The MBean must have a listener
     * that exactly matches the given <code>listener</code>, <code>filter</code>
     * , and <code>handback</code> parameters. If there is more than one such
     ...
     * <tt>CascadingProxy</tt>'s <tt>ObjectName</tt>. This implementation simply
     * substitute the notification source by this <tt>CascadingProxy</tt>'s
     * <tt>ObjectName</tt>, iff the source is the <tt>ObjectName</tt> of the
     * source MBean.
     * 
     * @return <var>notif</var>, after the source has been possibly substituted.
     **/
