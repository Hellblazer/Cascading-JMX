...
/**
 * The <tt>CascadingServiceMBean</tt> is a high level service MBean that makes
 * it possible to remotely configure <tt>CascadingAgents</tt>. This MBean makes
 * it possible to <i>mount</i> a partial view of a <i>source MBeanServer</i>
 * known by its <tt>JMXServiceURL</tt> into the <i>target MBeanServer</i> of
 * this <tt>CascadingServiceMBean</tt>.
 * <p>
 * The target <tt>MBeanServer</tt> of a <tt>CascadingServiceMBean</tt> is
 * usually the <tt>MBeanServer</tt> in which that <tt>CascadingServiceMBean</tt>
 * is registered. It is recommended to instantiate at most one
 * <tt>CascadingServiceMBean</tt> per target MBeanServer. If no
 * <tt>ObjectName</tt> is specified when registering the
 * <tt>CascadingServiceMBean</tt>, then the <tt>CascadingServiceMBean</tt> will
 * supply its own default name: <tt>{@link #CASCADING_SERVICE_DEFAULT_NAME 
 ...
 * <li>The <var>userData</var> is the source <tt>JMXConnectionNotification</tt></li>
 * </ul>
 * </p>
 * <p>
 * If the cascading is unmounted ({@link #unmount unmount} was called) then the
 * <tt>CascadingServiceMBean</tt> will emit a {@link JMXConnectionNotification}
 ...
 * <li>The <var>connectionId</var> is the <var>mountPointID</var> that was
 * returned by the the {@link #mount mount} method</li>
 * <li>The <var>userData</var> is either <tt>null</tt>, or the exception thrown
 * by {@link #unmount unmount}</li>
 * </ul>
 * </p>
 * <p>
 * <b>Note:</b> In this implementation, the {@link #mount mount} operation does
...
public class CascadingService implements CascadingServiceMBean,
    ...
    class MountPoint {
        ...
        public void checkConnection() throws IOException {
            ...
            final MBeanServerConnection mbs = sourceConnectionFactory.getMBeanServerConnection();
            if (mbs == null) {
                throw new IOException("not connected");
            }
        ...
        public boolean isClosed() {
            if (agent != null) {
                try {
                    agent.getConnectionFactory().getMBeanServerConnection().getDefaultDomain();
        ...
        public void unmount() throws IOException {
            ...
            try {
                sourceConnectionFactory.removeConnectionNotificationListener(listener,
                                                                             null,
                                                                             this);
            } catch (Exception x) {
                failure = x;
            ...
            if (failure != null) {
                final IOException io = new IOException(mountPointID + ": "
                                                       + failure, failure);
                throw io;
            }
        }
    ...
    private static final String[]              jmxConnectionNotificationTypes = {
            CASCADING_FAILED_NOTIFICATION, CASCADING_STOPPED_NOTIFICATION    };
     ...
     * <tt>MBeanServer</tt> for this service will be the <tt>MBeanServer</tt> in
     * which this object is registered.
     * 
     **/
    ...
    /**
     * Instantiate a new <tt>CascadingService</tt> for the specified target
     * <tt>MBeanServer</tt>. The target <tt>MBeanServer</tt> for this service
     * will be the specified <var>targetMBS</var>.
     * 
     * @param targetMBS
     *            The <i>target MBeanServer</i> for this cascading service.
     **/
    public CascadingService(MBeanServer targetMBS) {
        ...
        mountMap = new HashMap<String, MountPoint>();
    }
    ...
    public final synchronized String mount(JMXServiceURL sourceURL,
        ...
        final JMXConnector sourceConnector = connectSource(sourceURL,
                                                           sourceMap,
                                                           mpt.mountPointID);
     ...
     * registered in the MBean server. If the target MBeanServer supplied at
     * construction time was null, then <var>server</var> becomes the target
     * <tt>MBeanServer</tt>.
     * 
     * @param server
     ...
     * 
     * @param name
     *            The object name of the MBean. If <var>name</var> is
     *            <tt>null</tt> then
     *            {@link CascadingServiceMBean#CASCADING_SERVICE_DEFAULT_NAME}
     *            is returned.
     * 
     * @return The given <var>name</var>, or if <tt>null</tt> the default name
     *         for the <tt>CascadingService</tt>.
     * 
     * 
     * @exception IllegalArgumentException
     *                if no target <tt>MBeanServer</tt> was specified in the
    ...
    public final boolean unmount(String mountPointID) throws IOException {
        ...
        try {
            synchronized (this) {
                mpt = mountMap.remove(mountPointID);
                if (mpt == null) {
                    return false;
                }
            ...
            if (mpt != null) {
                ...
                final String message = failure == null ? ": succesfully unmounted"
                                                      : ": unmounted with errors: "
                                                        + failure;
                JMXConnectionNotification stopped = new JMXConnectionNotification(
                                                                                  type,
                                                                                  this,
                                                                                  mpt.mountPointID,
                                                                                  newSequenceNumber(),
    ...
    private void handleJMXCN(Notification nt, Object handback) {
        ...
        if (closed) {
            ...
            JMXConnectionNotification failed = new JMXConnectionNotification(
                                                                             type,
                                                                             this,
                                                                             mpt.mountPointID,
                                                                             newSequenceNumber(),
     ...
     * source <tt>MBeanServer</tt>. This is the <var>sourceConnector</var> that
     * will be passed to {@link #createCascadingAgent createCascadingAgent} and
     * {@link #terminate terminate}. <br>
     ...
     *            {@link JMXConnectorFactory#connect(JMXServiceURL,Map)} method,
     *            in order to connect to the source <tt>MBeanServer</tt>. This
     *            parameter can be null.
     *            <p>
     * @param mountPointID
     *            The <var>mountPointID</var> identifying the mount point for
     *            which this connector is created. This is the
     *            <var>mountPointID</var> that will be returned by the
     *            {@link #mount mount} operation.
     *            <p>
     * @exception IOException
     *                if the JMXConnector cannot be connected - see
    ...
    /**
     * Creates a new cascading agent for implementing the {@link #mount mount}
     * operation. The returned <tt>CascadingAgent</tt> is not expected to be
     * already started: {@link #mount mount} will later call
     * <tt>start(false)</tt> on this object.
     * <p>
     * By default this method creates a
     * {@link BasicMBeanServerConnectionFactory} from the
     * <var>sourceConnector</var> and then instantiate a new
     * {@link ProxyCascadingAgent}.
     * 
     * @param sourceConnector
     *            A connected <tt>JMXConnector</tt> for communicating with the
     *            source <tt>MBeanServer</tt>. This is the <tt>JMXConnector</tt>
     *            that was returned by {@link #connectSource connectSource}.
     *            <p>
     * @param sourcePattern
     *            An <tt>ObjectName</tt> pattern that must be satisfied by the
     *            <tt>ObjectName</tt>s of the source MBeans. This is the
     ...
     *            This is the <var>mountPointID</var> that will be returned by
     *            the {@link #mount mount} operation.
     *            <p>
     ...
     * 
     * @param agent
     *            The <tt>CascadingAgent</tt> created for this mount point by
     *            {@link #createCascadingAgent createCascadingAgent}. Can be
     *            <tt>null</tt> if <tt>terminate</tt> is called before the agent
     *            was created. <br>
     *            When not <tt>null</tt>, the <tt>terminate</tt> method calls
     *            <tt>agent.stop()</tt>.
     *            <p>
     * @param sourceConnector
     *            The <tt>JMXConnector</tt> created for this mount point by
     *            {@link #connectSource connectSource}. <br>
     *            If <var>sourceConnector</var> is not <tt>null</tt>, the
     *            <tt>terminate</tt> method calls
     *            <tt>sourceConnector.close()</tt>.
     *            <p>
     * @param mountPointID
     *            The <var>mountPointID</var> identifying the mount point being
     *            unmounted. This is the <var>mountPointID</var> that was
     *            returned by the corresponding {@link #mount mount} operation.
     *            <p>
     * @exception IOException
     *                If an IOException exception occurs while stopping the
     *                agent or closing the connector. <br>
     *                <b>Note:</b> the <tt>terminate</tt> method should always
